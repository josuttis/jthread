...
{\color{insertnote}This color signals differences to class \tcode{std::thread}.} ]
{\color{insertcolor}
...
\begin{itemdescr}
  ...
  \pnum\postconditions \tcode{get_id() == id()}
        {\color{insertnote} and \tcode{itoken.valid() == false}}.
\end{itemdescr}
...
If these functions return with \tcode{cv_status::interrupted}
their synchronization behavior is as though they called \tcode{is_interrupted()}.
...
\indexlibrarymember{wait_or_throw}{condition_variable}%
\begin{itemdecl}
void wait_or_throw(unique_lock<mutex>& lock);
\end{itemdecl}
\begin{itemdescr}
  \pnum\effects Like \tcode{wait(lock)} with the following modifications:
  \begin{itemize}
    \item ...
    \item The function will unblock when signaled by a call to \tcode{notify_one()}
          or a call to \tcode{notify_all()},
          when \tcode{this_thread::is_interrupted() == true},
          or spuriously.
  \end{itemize}

  \pnum\throws \tcode{interrupted} if \tcode{this_thread::is_interrupted() == true}.

  \pnum\sync In that case their synchronization behavior is as though they called \tcode{is_interrupted()}.
\end{itemdescr}
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\effects Like \tcode{wait(lock, pred)} with the following modifications:
  
  \pnum\throws Any exception thrown by \tcode{pred}
              or \tcode{interrupted} if \tcode{this_thread::is_interrupted() == true}.
...
\begin{itemdescr}
  ...
  \pnum\returns \tcode{cv_status::interrupted} if \tcode{this_thread::is_interrupted() == true},
                otherwise \tcode{cv_status::no_timeout}.
