...
\begin{itemdescr}
  ...
  \pnum\returns \tcode{false} if:
  \begin{itemize}
   \item \tcode{*this} does not own a stop state, or
...
\pnum
If \tcode{stop_callback} is instantiated with an argument for the template parameter
\tcode{Callback} that does not satisfy \tcode{Invocable} or does not model
\tcode{Destructible}, the program is ill-formed.

\pnum
If \tcode{stop_callback} is instantiated with an argument for the template parameter
\tcode{Callback} that does not model \tcode{Invocable} or does not model \tcode{Destructible},
the behaviour is undefined.

{\color{blue} ??? OR (see LWG3193): }
