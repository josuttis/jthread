...
\pnum
Callbacks registered via a \tcode{stop_callback} object is called when
...
\pnum
Concurrent calls to \tcode{request_stop()}, \tcode{stop_requested()},
...
All \tcode{stop_source}s sharing the same shared stop state can request a stop.
...
\begin{itemdescr}
  ...
  \pnum\effects If \tcode{!stop_possible()} or \tcode{stop_requested()} the call has no effect. 
                Otherwise, atomically requests a stop so that \tcode{stop_requested() == true}
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\returns \tcode{!lhs.stop_possible() \&\& !rhs.stop_possible()} or
...
\begin{itemdescr}
  \pnum\returns \tcode{true} if \tcode{request_stop()} was called on and
                associated \tcode{stop_source}, otherwise \tcode{false}, executed atomically.
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\returns \tcode{true} if both \tcode{lhs} and \tcode{rhs} both
                have no shared stop state or refer to the same stop state
                (copied or moved from the same initial \tcode{stop_source} object).
               % \tcode{true} if both \tcode{lhs} and \tcode{rhs} both
               % have no shared stop state or refer to the same stop state
\end{itemdescr}
