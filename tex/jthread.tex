...
\pnum
A \tcode{stop_token} can be passed to an operation which can
\begin{itemize}
 \item either actively poll the token to check if there has been a stop request or
...
withdrawn (a second and subsequent stop requests have no effect).
...
\begin{itemdescr}
  \pnum\effects Initializes \tcode{callback} with \tcode{static_cast<C\&\&>(cb)}.
                ...
                such that \tcode{static_cast<Callback\&\&>(callback)} is invoked
                ...
                state as \tcode{st}.
                If invoking the callback exists via an exception 
...
\begin{itemdescr}
  \pnum\effects Unregisters the callback from the associated shared stop state.
                ...
                the destructor does not block (\ref{defns.block}) waiting for the return from
...
this indicates that a \tcode{stop_source} object not sharing a stop state is constructed.
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\postconditions \tcode{stop_possible() == true \&\& stop_requested() == false}.
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\postconditions \tcode{stop_possible()} is \tcode{false}.
                \begin{note} No resources have to be associated for the state.  \end{note}
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\postconditions \tcode{*this} contains the old value of \tcode{rhs} and
                        \tcode{rhs.stop_possible()} is {false}.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \pnum\effects Let \placeholder{x} be the state observed by \tcode{stop_requested()}.
                ...
                % TODO: NEU
                If an invocation of a callback exists via an exception 
                then \tcode{terminate()} is called.
                \begin{note} A stop request includes notifying all condition variables
                             of type \tcode{condition_variable_any}
                             temporarily registered during an interruptible wait (\ref{thread.condvarany.interruptwait}).
                             \end{note}
...
\begin{itemdescr}
  ...
  \pnum\postconditions \tcode{get_id() == id()}
        {\color{diffcolor} and \tcode{ssource.stop_possible()} is \tcode{false}}.
\end{itemdescr}
...
\pnum\postconditions \tcode{get_id() != id()}.
{\color{diffcolor}
                     \tcode{ssource.stop_possible()} is \tcode{true}.
}
                     \tcode{*this} represents the newly started thread.
{\color{diffcolor}
\begin{note} Note that the calling thread can perform a stop request only once,
...
%{\color{diffcolor}
 \pnum\effects Registers \tcode{*this} to get notified on a stop request on \tcode{stoken}
