...
Calls to \tcode{interrupt()} and \tcode{is_interrupted()}
are atomic operations(6.8.2.1p3 \ref{intro.races})
on the interrupt state contained in the \tcode{interrupt_token} object.
...
\tcode{is_interrupted()} that observes the interrupt.
...
\pnum\effects
{\color{diffcolor} initializes \tcode{itoken} with \tcode{false} and
}
constructs an object of type \tcode{jthread}. The new thread of execution executes
either 
{\color{diffcolor}
        \tcode{%
                \placeholdernc{INVOKE}(\brk{}%
...
[{\itshape{}Editorial note:} Because all signatures here in the effects clause call
        \tcode{is_interrupted()}, we don't need wording
