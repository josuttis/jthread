...
namespace std {
  ...
  public:
    ...
    explicit stop_callback(const stop_token& it, Callback&& cb)
        noexcept(std::is_nothrow_move_constructible_v<Callback>);
    explicit stop_callback(stop_token&& it, Callback&& cb)
        noexcept(std::is_nothrow_move_constructible_v<Callback>);
    ~stop_callback();
...
\indexlibrarymember{operator==}{stop_source}%
\begin{itemdecl}
bool operator== (const stop_source& lhs, const stop_source& rhs);
\end{itemdecl}
\begin{itemdescr}
