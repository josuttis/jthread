...
\end{itemdecl}
\begin{itemdescr}
  \pnum\postconditions \tcode{stop_possible() == true \&\& stop_requested() == false} is \tcode{true}.
...
exists via an exception, \tcode{terminate()} is called.
...
\begin{itemdescr}
\pnum
If \tcode{joinable()} is \tcode{true}, calls \tcode{request_stop()} and \tcode{join()}.
