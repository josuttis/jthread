...
\begin{itemize}
 \item Several editorial fixes requested by Jonathan Wakely.
\end{itemize}
...
that the constructor performs a decayed copy of the callback argument.
And because the constructors are function templates,
...
\endhead
 ...
 \tcode{auto stop = [] \{ ... \}; } \br
 \tcode{stop_callback  cb\{token, move(stop)\}; }
  &
        Moves \tcode{stop} into \tcode{cb}.
	\\ \rowsep

 \tcode{auto stop = [] \{ ... \}; } \br
 \tcode{stop_callback cb\{token, ref(stop)\}; }
  &
        Captures lvalue reference to stop in \tcode{cb}.
        Deduces Callback arg to \tcode{reference_wrapper<lambda>}
	
        %???: With Jonathan’s deduction guide suggestion this would deduce
 ...
 \tcode{\}();     }
  &	
        ...
        Callback arg deduced to be \tcode{function<void()>}
        \\ \rowsep
 ...
 \tcode{stop_callback<myCallback> cb\{token, i\}; }
  &
        ...
        Directly constructs \tcode{myCallback} member of \tcode{cb} from \tcode{i}.
        \\ \rowsep

 \tcode{explicitArg e; } \br
 \tcode{stop_callback<myCallback> cb\{token, e\}; }
  &
        %Ill-formed. Cannot implicitly convert ‘e’ to ‘myCallback&&’.
        %
        %Either ill-formed (if we constrain constructor parameter with ConvertibleTo<Callback>.
        %Or it is valid and directly constructs myCallback member of ‘cb’ from ‘e’ if we constrain constructor
        %parameter with Constructible<Callback, Arg>, possibly marking stop_callback constructor as explicit(!ConvertibleTo<Arg, Callback>)
        %Or it is valid and directly constructs myCallback member of ‘cb’ from ‘e’ if we constrain constructor parameter with Constructible<Callback, Arg>, possibly marking stop_callback constructor as explicit(!ConvertibleTo<Arg, Callback>)
        Directly constructs myCallback member of \tcode{cb} from \tcode{e} if we constrain constructor.
        \\ \rowsep
